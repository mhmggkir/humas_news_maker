...
system_prompt = f"Buat berita yang di kirimkan oleh user menjadi berita yang menarik dan bermodel paragraf, gunakan format json di setiap style font, gunakan struktur seperti berikut {str(news_structure)}, pastikan isi berita dapat pas pada ukuran kertas yang diberikan (Inch), ukuran font (pt), gunakan bahasa Indonesia, jika berita yang diberikan oleh user kosong maka tuliskan gagal di setiap content, berikan dateline di awal berita yang mengandung tempat dan sumber berita, jangan berikan tulisan copyright, buat minimal 5 paragraf, buat masing masing paragraf berisi 5 baris, gunakan gaya bahasa seperti artikel ilmiah populer"
...
def get_news(url):
  ...
  headline = element.find_all("p", {"class": None})
  res_arr = []
  image = None
  for i in range(3):
    try:
  ...
  if res_arr == []:
    ...
    print("News has gotten")
  return {"news": " ".join(res_arr), "img": image}

def create_document(data, img, news_length, news_index):
...
def main():
  url_arr = []
  while(True):
    url = input("News URL (type q to quit): ")
    if url == "q": break
    url_arr.append(url)
  ...
  for index, url in enumerate(url_arr):
    ...
    for data in news_result:
      print(data["title"]["content"])
      create_document(data=data, img=data["image"], news_length=len(url_arr), news_index=index)
    print(f"News ({index+1}/{len(url_arr)}) created")
    time.sleep(0.3)
